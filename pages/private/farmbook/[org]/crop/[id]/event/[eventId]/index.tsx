...
function EventDetailPageWrapper() {
  ...
  const { org, id: cropId, eventId } = router.query;
...
function EventDetailPage(props: InternalPageProps) {
  ...
  const { isLoading: isLoadingEvidences, data: cropEvidenceList } = useFetch<any[]>(
    `${apiPrefix}/event?cropId=${cropId}&category=Submission`,
  );
  ...
  React.useEffect(() => {
    setTitleBarData({
      ...titleBarData,
      subTitle:
        data?.crop.name + ' • ' + data?.crop.landParcel.name + ' • ' + data?.crop?.farmer?.name,
      mainBtnTitle:
      ...
      isSubBtnPresent:
        (!data.status ||
          ...
          data.status === 'Draft' ||
          data.status === 'Review Failed' ||
          data.status === 'Validation Failed') &&
        !data.locked,
      isViewDeleteBtnsPresent: !data.locked,
    });
  }, [data, editMode]);
  ...
  return (
    <EventAttachmentDataContext
      url={`/event?cropId=${cropId}&category=Submission`}
    >
      <TitleBarGeneric
