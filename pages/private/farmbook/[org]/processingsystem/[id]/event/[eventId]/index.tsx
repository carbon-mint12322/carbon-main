...
import { WfEventSubmission, submitWfEvent } from '~/frontendlib/model-lib/workflow';
...
function EventDetailPageWrapper() {
  ...
  const { org, id: processingSystemId, eventId } = router.query;
  ...
  return (
    <EventDetailPage
      org={org as string}
      processingSystemId={processingSystemId as string}
...
function EventDetailPage(props: InternalPageProps) {
  ...
  const { isLoading: isLoadingEvidences, data: lpEvidenceList } = useFetch<any[]>(
    `${apiPrefix}/event/list?processingSystemId=${processingSystemId}&category=Submission`,
  );
  ...
  const getProcessingSystemData = async () => {
    try {
      const res: {
        data: any;
      } = await axios.get(apiPrefix + `/processingsystem/${processingSystemId}`);
      return res;
    } catch (error) {
      console.error(error);
    }
  ...
  React.useEffect(() => {
    getProcessingSystemData().then((res: any) => {
      setProcessingSystemData(res?.data);
    });
    setTitleBarData({
      ...titleBarData,
      title: eventData?.processingSystemId
        ? 'Processing System Event Details'
        : 'Land Parcel Event Details',
      mainBtnTitle:
        eventData?.status == 'Approved' ? 'Approved' : statusStep[eventData?.status]?.buttonLabel,
      subBtnTitle: editMode ? 'Cancel Edit' : 'Edit',
      isSubBtnPresent:
        (!eventData.status ||
          eventData.status === 'editable' ||
          eventData.status === 'Draft' ||
          eventData.status === 'Review Failed' ||
          eventData.status === 'Validation Failed') &&
        !eventData.locked,
      isViewDeleteBtnsPresent: !eventData.locked,
    });
  }, [eventData, editMode]);
...
function DetailView({
  ...
  async function solarDryerFilter() {
    const res: {
      data: any;
    } = await axios.get(getAPIPrefix() + `/landparcel/${processingSystemData.landParcel}`);
  ...
  const formContext: any = {
    getApiUrl,
    foreignObjectLoader:
      eventName === 'psSolarDryerLoadEvent'
        ? solarDryerFilter
        : eventName === 'psOutputProcessingEvent'
          ? outputProcessingUnitFilter
          : eventName === 'psCompostingInputEvent' ||
            eventName === 'psCompostingHarvestEvent'
