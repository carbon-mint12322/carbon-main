...
import { ProcessingSystem } from '~/frontendlib/dataModel';
...
export default function CreateEvent(props: any) {
  ...
  const [processingSystemData, setProcessingSystemData] = useState<ProcessingSystem>();
  ...
  const { isLoading: loading, data } = useFetch<any[]>(
    `${getAPIPrefix()}/event?processingSystemId=${router.query.id}&category=Submission`,
  );

  const getProcessingSystemData = async () => {
    try {
      const res: {
        data: any;
      } = await axios.get(getAPIPrefix() + `/processingsystem/${router.query.id}`);
      return res;
    } catch (error) {
      console.error(error);
    }
  ...
  const setCreateEventTitle = () => {
    getProcessingSystemData().then((res) => {
      setProcessingSystemData(res?.data);
      setTitleBarData({
        ...titleBarData,
        subTitle:
          res?.data?.name + ' • ' + res?.data?.category + ' • ' + res?.data?.landParcels[0].name,
      });
    });
  };
  ...
  return (
    <EventAttachmentDataContext
      url={`/event?processingSystemId=${router.query.id}&category=Submission`}
    >
      <TitleBarGeneric
      ...
      >
        ...
        >
          <AppConfigContext.Provider value={props?.pageConfig}>
            <Content
              pageProps={props}
              processingSystemData={processingSystemData}
              selectedImages={selectedImages}
            />
...
const workflowNames = [
  [
    'psSolarDryerLoadEventLifecycle',
    'psCompostingInputEventLifecycle',
    'psCompostingHarvestEventLifecycle',
    'psOutputProcessingEventLifecycle',
  ],
];

function Content(props: any) {
  const { selectedImages, processingSystemData } = props;
  ...
  const processingSystemId: string | undefined = router?.query?.id as string;

  const handleSubTitleChange = (evt: any) => setSelectedSubEvent(evt.target.value);

  if (!processingSystemId) {
    return <div> Processing System ID is required </div>;
  }

  if (!wfName) {
    return <div> Event is not defined </div>;
  }
  const Events = ['Processing Events'];
  ...
  function createEvent(input: any) {
    return mutate({
      ...input,
      domainSchemaName: 'processingsystem',
      domainInstanceId: processingSystemId,
    });
  }

  const landParcel = processingSystemData?.landParcel;

  async function solarDryerFilter() {
    const res: {
      data: any;
    } = await axios.get(getAPIPrefix() + `/landparcel/${landParcel}`);
  ...
  const formContext: any = {
    getApiUrl,
    foreignObjectLoader:
      wfName === 'psSolarDryerLoadEventLifecycle'
        ? solarDryerFilter
        : wfName === 'psOutputProcessingEventLifecycle'
          ? outputProcessingUnitFilter
          : wfName === 'psCompostingInputEventLifecycle' ||
            wfName === 'psCompostingHarvestEventLifecycle'
        ...
        <Suspense fallback={<NextProgress delay={10} options={{ showSpinner: true }} />}>
          <MutationWrapper mutation={submitMutation}>
            <EventEditor
              ...
              domainObjectId={processingSystemId}
              evidences={evidences}
            />
          </MutationWrapper>
        </Suspense>
