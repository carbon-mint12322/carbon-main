...
function EventDetailPageWrapper() {
  ...
  const { org, id: productionSystemId, eventId } = router.query;
  ...
  return (
    <EventDetailPage
      org={org as string}
      productionSystemId={productionSystemId as string}
...
function EventDetailPage(props: InternalPageProps) {
  ...
  const { isLoading: isLoadingEvidences, data: lpEvidenceList } = useFetch<any[]>(
    `${apiPrefix}/event/list?productionSystemId=${productionSystemId}&category=Submission`,
  );
  ...
  React.useEffect(() => {
    setTitleBarData({
      ...titleBarData,
      title: eventData?.productionSystemId
        ? 'Production System Event Details'
