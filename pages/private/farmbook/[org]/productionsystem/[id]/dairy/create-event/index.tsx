...
const workflowNames = [
  ['cowFeedPSLifecycle', 'cowMedicationPSLifecycle', 'cowVaccinationPSLifecycle'],
  ['cowMilkingPSLifecycle'],
  ['cowSalePSLifecycle'],
];
...
function Content(props: any) {
  ...
  const Events = ['Health and Medication', 'Milk Production', 'Sales'];

  const EventSubTitles = [['Feed', 'Medication', 'Vaccination'], ['None'], ['None']];
  ...
  async function cowsFilter() {
    const res: {
      data: any;
    } = await axios.get(getAPIPrefix() + `/cow`);

    // Filter the data array based on the productionSystemId
    const filteredData = res.data.filter((cow: any) => cow.productionSystem === productionSystemId);
  ...
  const formContext: any = {
    getApiUrl,
    foreignObjectLoader:
      wfName === 'cowFeedPSLifecycle' ||
        wfName === 'cowMedicationPSLifecycle' ||
        wfName === 'cowVaccinationPSLifecycle' ||
        wfName === 'cowMilkingPSLifecycle' ||
        wfName === 'cowSalePSLifecycle'
        ? cowsFilter
        : undefined,
  };
