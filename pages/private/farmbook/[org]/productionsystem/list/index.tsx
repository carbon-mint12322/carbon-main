...
function ProductionsystemsList() {
  ...
  const API_URL = getAPIPrefix() + '/productionsystem';
  const { isLoading: loading, data, reFetch } = useFetch<any>(API_URL);

  React.useEffect(() => {
    setTitleBarData({
      ...titleBarData,
      subTitle: `Showing ${data?.length || 0} Production Systems in total`,
    });
  }, [data]);

  const handleRowClick = (params: GridRowParams) => {
    changeRoute(`/productionsystem/${params?.row?.id}`);
  };
...
export default withAuth(ProductionsystemsList);
