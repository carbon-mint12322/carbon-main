...
import { isAudioUrl, isDocumentUrl, isImgUrl } from '~/frontendlib/util';
...
function EventDetailPageWrapper() {
  ...
  const { org, id: collectiveId, eventId } = router.query;
  ...
  return (
    <EventDetailPage
      org={org as string}
      collectiveId={collectiveId as string}
...
function EventDetailPage(props: InternalPageProps) {
  ...
  const { isLoading: isLoadingEvidences, data: operatorEvidenceList } = useFetch<any[]>(
    `${apiPrefix}/event/list?category=Submission`,
  );
  ...
  const saveMutation = useMutation(async (data: any) => {
  ...
  function saveChanges(updatedData: any) {
    ...
    return saveMutation.mutate({ details: updatedData, evidences });
  }
  ...
  return (
    ...
    >
      ...
      <Grid item xs={7}>
        ...
        >
          <Box>
            <MutationWrapper mutation={saveMutation}>
              <DetailView
                readonly={!editMode}
                data={data}
