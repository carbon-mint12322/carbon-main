...
function EventDetailPageWrapper() {
  ...
  const { org, id: cowId, eventId } = router.query;
...
function EventDetailPage(props: InternalPageProps) {
  ...
  const { setTitleBarData, setHandleMainBtnClick, setHandleSubBtnClick, setHandleDeleteBtnClick } =
    useTitleBar();
  ...
  const { isLoading: isLoadingEvidences, data: cowEvidenceList } = useFetch<any[]>(
    `${apiPrefix}/event?productionSystemId=${cowId}&category=Submission`,
  );
  ...
  React.useEffect(() => {
    ...
    setHandleSubBtnClick(toggleEditMode);
    setHandleDeleteBtnClick(deleteEvent);
  }, [eventData, editMode]);
