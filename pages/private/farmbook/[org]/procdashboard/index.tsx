...
export interface Data {
  ...
  totalSolarDryers: number;
  totalCompostingUnits: number;
  totalProducts: number;
  totalProductBatches: number;
...
function ProcDashboard(props: ProcDashboardProps) {
  ...
  const countCardData = [
    ...
    {
      id: 1,
      count: data?.totalCrops || 0,
      title: 'Crops',
      icon: cropImg,
    },
  ];

  const procCountCardData = [
    {
      id: 4,
      count: data?.totalProcessors || 0,
      title: 'Processors',
      icon: userCircularOrange,
    },
    {
    ...
    {
      id: 2,
      count: data?.totalSolarDryers || 0,
      title: 'Solar Dryers',
      icon: landParcels,
    },
    {
      id: 5,
      count: data?.totalCompostingUnits || 0,
    ...
    {
      id: 1,
      count: data?.totalProductBatches || 0,
      ...
      <CircularLoader value={loading}>
        <>
          <Grid item container spacing={1}>
            {procCountCardData?.map((procCountCard) => (
              <Grid key={procCountCard.id} container item rowSpacing={1} lg={2} xs={6} md={6}>
                <Box width='100%'>
                  <CountCard
                    count={procCountCard.count}
                    icon={procCountCard.icon}
                    title={procCountCard.title}
                  />
                </Box>
              </Grid>
            ))}
...
export default withAuth(ProcDashboard);
