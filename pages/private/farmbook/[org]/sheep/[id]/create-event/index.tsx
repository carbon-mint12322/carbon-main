...
export default function CreateEvent(props: any) {
  ...
  const getSheepData = async () => {
    try {
      const res: {
        data: any;
      } = await axios.get(getAPIPrefix() + `/sheep/${router.query.id}`);
      return res;
    } catch (error) {
      console.error(error);
    }
  ...
  const setCreateEventTitle = () => {
    getSheepData().then((res) => {
      const sheepData = res?.data?.[0];
      setTitleBarData({
        ...initialTitleBarContextValues.titleBarData,
        isTitleBarPresent: true,
        title: 'New Event',
        subTitle:
          sheepData?.name + ' • ' + sheepData?.landParcel?.name + ' • ' + sheepData?.farmer?.name,
        subBtnColor: 'grey',
        isTitlePresent: true,
      ...
      getSubmissionsData(sheepData?.productionSystem?.id).then((res) => {
  ...
  return (
    <EventAttachmentDataContext
      url={`/event?productionSystemId=${sheepData?.productionSystem?.id}&category=Submission`}
...
const workflowNames = [
  [
    'sheepBirthLifecycle',
    'sheepBreedingLifecycle',
    'sheepCalvingLifecycle',
    'sheepWeaningLifecycle',
  ],
  [
    'sheepWeightGrowthLifecycle',
    'sheepFeedLifecycle',
    'sheepMedicationLifecycle',
    'sheepVaccinationLifecycle',
  ],
...
function Content(props: any) {
  ...
  const sheepId: string | undefined = router?.query?.id as string;

  const handleSubTitleChange = (evt: any) => setSelectedSubEvent(evt.target.value);

  if (!sheepId) {
    return <div> Sheep ID is required </div>;
  }
  ...
  const Events = ['Reproduction', 'Health and Medication', 'Sales'];
  ...
  function createEvent(input: any) {
    return mutate({ ...input, domainSchemaName: 'sheep', domainInstanceId: sheepId });
  }
