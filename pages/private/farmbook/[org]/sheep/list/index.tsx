...
const styles = {
  ...
  sheepAndLandParcelSubtitle: { color: 'text.disabled', wordWrap: 'break-word' },
  renderActionCell: {
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
...
export default function Sheeps() {
  ...
  const [titleBarData, setTitleBarData] = useState<any>({
    ...
    title: 'Sheep',
    mainBtnTitle: 'Add Sheep',
    isTitlePresent: true,
    isSubTitlePresent: true,
    isSearchBarPresent: true,
    isMainBtnPresent: false,
  });
  const API_URL = getAPIPrefix() + '/sheep';
  const { isLoading: loading, data, reFetch } = useFetch<any>(API_URL);

  React.useEffect(() => {
    setTitleBarData({
      ...titleBarData,
      subTitle: `Showing ${data?.length || 0} sheep in total`,
    });
  }, [data]);

  const handleRowClick = (params: any) => {
    changeRoute(`/sheep/${params?.row?.id}`);
  };
  ...
  const renderCropAndLandParcelCell = (data: any) => {
    return (
      <CropAndLandParcelCell
        ...
        subTitleSx={styles.sheepAndLandParcelSubtitle}
      />
    );
  };
    ...
    return (
      <Box component={'div'} sx={styles.renderActionCell}>
        <Button
          onClick={(e) => {
            e.stopPropagation();
            changeRoute(`/sheep/${data?.row?.id}/create-event`);
          }}
        >
          {'Create Event'}
        </Button>
