...
export default function CreateEvent(props: any) {
  ...
  let aquacropData: any;

  const { getAPIPrefix } = useOperator();

  const getAquaCropData = async () => {
    try {
      const res: {
        data: any;
      } = await axios.get(getAPIPrefix() + `/aquacrop/${router.query.id}`);
      return res;
    } catch (error) {
      console.error(error);
    }
  ...
  const setCreateEventTitle = () => {
    getAquaCropData().then((res) => {
      aquacropData = res?.data?.[0];
      setTitleBarData({
        ...
        subTitle:
          aquacropData?.fbId +
          ' • ' +
          aquacropData?.landParcel?.name +
          ' • ' +
          aquacropData?.farmer?.name,
        subBtnColor: 'grey',
        isTitlePresent: true,
      ...
      getSubmissionsData(aquacropData.productionSystem).then((res) => {
  ...
  return (
    <EventAttachmentDataContext
      url={`/event?productionSystemId=${aquacropData?.productionSystem}&category=Submission`}
...
const workflowNames = [
  ['aquacropPondPreparationLifecycle'],
  ['aquacropSeedProcurementLifecycle', 'aquacropStockingLifecycle'],
  ['aquacropFeedLifecycle'],
  ['aquacropMedicationLifecycle'],
  ['aquacropGrowthMeasurementLifecycle'],
  ['aquacropHarvestLifecycle', 'aquacropPostHarvestProcessingLifecycle'],
  ['aquacropAerationLifecycle'],
  [
    'aquacropFeedEfficiencyLifecycle',
    'aquacropWasteManagementLifecycle',
    'aquacropEnergyUsageLifecycle',
    'aquacropLandUseLifecycle',
  ],
  [
    'aquaCropWaterTestLifecycle',
    'aquacropOthersTestLifecycle',
    'aquacropAuditReportLifecycle',
    'aquacropInspectionLifecycle',
  ],
  ['aquacropInputLogLifecycle'],
  ['aquacropObservationsLifecycle'],
];
...
function Content(props: any) {
  ...
  const aquacropId: string | undefined = router?.query?.id as string;

  const handleSubTitleChange = (evt: any) => setSelectedSubEvent(evt.target.value);

  if (!aquacropId) {
    return <div> AquaCrop ID is required </div>;
  }
  ...
  const EventSubTitles = [
    ['None'],
    ['Seed Procurement', 'Stocking'],
    ['None'],
    ['None'],
    ['None'],
    ['Harvesting', 'Post-Harvest Processing'],
    ['Aeration'],
    ['Feed Efficiency', 'Waste Management', 'Energy Usage', 'Land Use'],
    ['Water Test', 'Others Test', 'Audit Report', 'Inspection Report'],
    ['None'],
    ['None'],
  ];
  ...
  function createEvent(input: any) {
    return mutate({ ...input, domainSchemaName: 'aquacrop', domainInstanceId: aquacropId });
  }
