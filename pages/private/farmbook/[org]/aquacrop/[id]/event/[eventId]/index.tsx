...
function EventDetailPageWrapper() {
  ...
  const { org, id: aquaId, eventId } = router.query;
...
function EventDetailPage(props: InternalPageProps) {
  ...
  const { isLoading: isLoadingEvidences, data: aquaEvidenceList } = useFetch<any[]>(
    `${apiPrefix}/event?productionSystemId=${data?.aquacrop?.productionSystems?.[0]._id}&category=Submission`,
  );
  ...
  React.useEffect(() => {
    setTitleBarData({
      ...
      title: 'Aquacrop Event Details',
      subTitle:
        data?.aquacrop?.fbId +
        ' • ' +
        data?.aquacrop?.cropType +
        ' • ' +
        data?.aquacrop?.landParcel.name,
      mainBtnTitle:
        data?.status == 'Approved' ? 'Approved' : statusStep[eventData?.status]?.buttonLabel,
  ...
  return (
    <EventAttachmentDataContext
      url={`/event?productionSystemId=${data?.aquacrop?.productionSystems?.[0]._id}&category=Submission`}
    >
      <TitleBarGeneric
