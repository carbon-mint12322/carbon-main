...
export default function CreateEvent(props: any) {
  ...
  const { isLoading: loading, data } = useFetch<any[]>(
    `${getAPIPrefix()}/event?category=Submission`,
  );

  const getCroppingSystemData = async () => {
    try {
      const res: {
        data: any;
      } = await axios.get(getAPIPrefix() + `/croppingsystem/${router.query.id}`);
      return res;
    } catch (error) {
      console.error(error);
    }
  ...
  const setCreateEventTitle = () => {
    getCroppingSystemData().then((res) => {
      const croppingsystemData = res?.data;
      setTitleBarData({
        ...titleBarData,
        subTitle:
          croppingsystemData?.name +
          ' • ' +
          croppingsystemData?.landParcels?.[0]?.name +
          ' • ' +
          croppingsystemData?.farmers?.[0]?.personalDetails?.firstName +
          ' ' +
          croppingsystemData?.farmers?.[0]?.personalDetails?.lastName,
      });
    });
  };
  ...
  return (
    <EventAttachmentDataContext url={`/event/list?category=Submission`}>
      <TitleBarGeneric
...
const workflowNames = [
  [
    'croppingSystemLandPrepBasicTillageWorkflow',
    'croppingSystemLandPrepDevelopmentWorkflow',
    'croppingSystemLandPrepLayoutWorkflow',
    'croppingSystemLandPrepOtherWorkflow',
  ],
  [
    'croppingSystemInterCulturalOperationsWeedingWorkflow',
    'croppingSystemInterCulturalOperationsMulchingWorkflow',
    'croppingSystemInterCulturalOperationsThinningWorkflow',
    'croppingSystemInterCulturalOperationsGapFillingWorkflow',
    'croppingSystemInterCulturalOperationsEarthingUpWorkflow',
    'croppingSystemInterCulturalOperationsPruningWorkflow',
    'croppingSystemInterCulturalOperationsCroppingSystemSpecificOperationWorkflow',
  ],
  ['croppingSystemIrrigationWorkflow'],
  [
    'croppingSystemIntegratedCropNutritionBioStimulantsWorkflow',
    'croppingSystemIntegratedCropNutritionFertilizersWorkflow',
    'croppingSystemIntegratedCropNutritionManureWorkflow',
    'croppingSystemIntegratedCropNutritionGeneralWorkflow',
  ],
  [
    'croppingSystemIntegratedCropCarePestManagementWorkflow',
    'croppingSystemIntegratedCropCarePreventiveMeasuresWorkflow',
    'croppingSystemIntegratedCropCareDiseaseManagementWorkflow',
    'croppingSystemIntegratedCropCareWorkflow',
  ],
];
...
function Content(props: any) {
  ...
  const croppingsystemId: string | undefined = router?.query?.id as string;

  const handleSubTitleChange = (evt: any) => setSelectedSubEvent(evt.target.value);

  if (!croppingsystemId) {
    return <div> CroppingSystem ID is required </div>;
  }
  ...
  const Events = [
    ...
    'Irrigation',
    'Integrated CroppingSystem Nutrition',
    'Integrated CroppingSystem Care',
  ];

  const EventSubTitles = [
    ['Basic Tillage', 'Development', 'Layout', 'Others'],
    [
      'Weeding',
      'Mulching',
      'Thinning',
      ...
      'Earthing up',
      'Pruning',
      'CroppingSystem specific operation',
    ],
    ['None'],
    ['Bio Stimulants', 'Fertilizers', 'Manure', 'General'],
    ['Pest Management', 'Preventive Measures', 'Disease Management', 'General'],
  ];
  ...
  function createEvent(input: any) {
    return mutate({ ...input, domainSchemaName: 'croppingsystem', domainInstanceId: croppingsystemId });
  }
        ...
        <Suspense fallback={<NextProgress delay={10} options={{ showSpinner: true }} />}>
          <MutationWrapper mutation={submitMutation}>
            <EventEditor
              ...
              wfName={wfName}
              domainObjectId={croppingsystemId}
              evidences={evidences}
            />
          </MutationWrapper>
        </Suspense>
