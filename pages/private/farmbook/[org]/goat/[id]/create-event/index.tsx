...
export default function CreateEvent(props: any) {
  ...
  const getGoatData = async () => {
    try {
      const res: {
        data: any;
      } = await axios.get(getAPIPrefix() + `/goat/${router.query.id}`);
      return res;
    } catch (error) {
      console.error(error);
    }
  ...
  const setCreateEventTitle = () => {
    getGoatData().then((res) => {
      const goatData = res?.data?.[0];
      setTitleBarData({
        ...initialTitleBarContextValues.titleBarData,
        isTitleBarPresent: true,
        title: 'New Event',
        subTitle:
          goatData?.name + ' • ' + goatData?.landParcel?.name + ' • ' + goatData?.farmer?.name,
        subBtnColor: 'grey',
        isTitlePresent: true,
      ...
      getSubmissionsData(goatData.productionSystem?.id).then((res) => {
  ...
  return (
    <EventAttachmentDataContext
      url={`/event?productionSystemId=${goatData?.productionSystem?.id}&category=Submission`}
...
const workflowNames = [
  ['goatBirthLifecycle', 'goatBreedingLifecycle', 'goatCalvingLifecycle', 'goatWeaningLifecycle'],
  [
    'goatWeightGrowthLifecycle',
    'goatFeedLifecycle',
    'goatMedicationLifecycle',
    'goatVaccinationLifecycle',
  ],
  ['goatMilkingLifecycle'],
  ['goatSaleLifecycle'],
];
...
function Content(props: any) {
  ...
  const goatId: string | undefined = router?.query?.id as string;

  const handleSubTitleChange = (evt: any) => setSelectedSubEvent(evt.target.value);

  if (!goatId) {
    return <div> Goat ID is required </div>;
  }
  ...
  function createEvent(input: any) {
    return mutate({ ...input, domainSchemaName: 'goat', domainInstanceId: goatId });
  }
