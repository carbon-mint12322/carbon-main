...
const SeedProcessingUnitsEditor = dynamic(
  import(
    '~/gen/data-views/landparcel_seedProcessingUnits/landparcel_seedProcessingUnitsEditor.rtml'
  ),
);

export default function SeedProcessingUnits({ data, lpData, modelName, reFetch, lpMap }: INestedProps) {
  const childResourceUri = 'seed-processing-units';
  const columnConfig = [
    {
      field: 'name',
      headerName: 'Name',
  ...
  return (
    <NestedResourceListComponent
      Editor={SeedProcessingUnitsEditor}
      data={data}
      parentData={lpData}
      reFetch={reFetch}
      modelName={modelName}
