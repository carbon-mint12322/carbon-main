...
export type CropEvent = CropActualEvent | CropPlanEvent;
...
export interface CropEventTimelineData extends TimelineItemBase<number> {
  id: number | string;
  bgColor?: string;
...
export interface CropEventTimelineGroup extends TimelineGroupBase {
  id: string;
  title: string;
  desc: string;
  color: string;
...
interface CropsTimelineProps {
  data: any;
  category: string;
  onItemSelection?: (eventItem?: CropEventTimelineData) => void;
  selectedEvent?: CropEventTimelineData;
...
const CropsTimeline = ({
  ...
  const [event, setEvent] = useState<CropEventTimelineData | null | undefined>(null);
  const [timelineItems, setTimelineItems] = useState<CropEventTimelineData[]>([]);
  const [timelineGroups, setTimelineGroups] = useState<CropEventTimelineGroup[]>([]);
  const { changeRoute } = useOperator();
  const [modalProps, setModalProps] = useState<IEventPlanEditModal>();
  const cropId = data.id;
  ...
  const triggerEditModal = (currentEventItem: CropEventTimelineData) => {
    // creating new instance
    const { currentPlanId, id: eventPlanId, group } = currentEventItem;
    if (group === 'actual') {
      changeRoute(`/${category}/${cropId}/event/${eventPlanId}`)
    }
    else {
      setModalProps({
        category,
  ...
  const handleOnItemSelect = (itemId: string | number) => {
    const eventItem: CropEventTimelineData[] = timelineItems?.filter((item) => item?.id === itemId);
    if (eventItem?.length) {
      // trigger edit modal
  ...
  const itemRenderer = ({
    ...
    itemContext,
    getItemProps,
  }: ReactCalendarItemRendererProps<CropEventTimelineData>) => {
  ...
  const groupRenderer = ({
    group: { title, desc },
  }: ReactCalendarGroupRendererProps<CropEventTimelineGroup>) => (
    <Box
      sx={{
      ...
      >
        {event?.group == 'plan' ? (
          <Grid container direction='column'>
            ...
            >
              {/*<Grid item xs={1.5}>*/}
              {/*  {event?.type && eventImage(event?.type, 32, 32)}*/}
              {/*</Grid>*/}
              <Grid item>
                <Typography
                  sx={{
...
export default CropsTimeline;

const eventGroupData: {
  [key: string]: CropEventTimelineGroup;
} = {
  plan: {
    id: 'plan',
    title: 'Plan',
...
const cropEventDataToCropEventTimelineDataFormatter = (
  ...
  items: CropEventTimelineData[];
  groups: CropEventTimelineGroup[];
} => {
  let timeLineData: CropEventTimelineData[] = [];
  let groupData: CropEventTimelineGroup[] = [];
  let progress: any;
  let currentPlanId: any;

  switch (category) {
    case 'crop':
      progress = data?.cropProgress;
      currentPlanId = data?.cropProgress?.plan.id;
      break;
    case 'poultry':
      progress = data?.poultryProgress;
      currentPlanId = data?.poultryProgress?.plan.id;
      break;
    case 'aquacrop':
      progress = data?.aquacropProgress;
      currentPlanId = data?.aquacropProgress?.plan.id;
      break;
    case 'cow':
      progress = data?.cowProgress;
      currentPlanId = data?.cowProgress?.plan.id;
      break;
    case 'collective':
      progress = data?.collectiveProgress;
      currentPlanId = data?.collectiveProgress?.plan.id;
      break;
    case 'goat':
      progress = data?.goatProgress;
      currentPlanId = data?.goatProgress?.plan.id;
      break;
    case 'sheep':
      progress = data?.sheepProgress;
      currentPlanId = data?.sheepProgress?.plan.id;
      break;
    default:
      break;
  }
  ...
  const getId = (event: CropEvent, defaultId: `index-${string}`): string => {
    ...
    if (!isCropCalendar) {
      if (group !== 'actual') {
        ...
        progress?.[group as keyof CropProgress]?.events?.forEach(
          (cropEvent: CropEvent, index: number) => {
            timeLineData.push({
              name: cropEvent.name,
