...
const CROPPING_SYSTEM_SCHEMA_ID = model2schemaId('croppingsystem');
const CroppingSystemApi = MongoAdapter.getModel(CROPPING_SYSTEM_SCHEMA_ID);

export const croppingSystemDetailsViewQuery = async (croppingSystemId: any) => {
  const dbResult = await CroppingSystemApi.aggregate([
    { $match: { _id: ObjectId(croppingSystemId) } },
    { $addFields: { croppingSystemId: { $toString: '$_id' } } },
    ...
    },
    {
      $lookup: {
        ...
        let: { croppingsystemId: '$croppingsystemId' },
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [
                  { $eq: ['$relatedTo.objectId', '$$croppingsystemId'] },
