...
export const sheepDetailsViewQuery = async (sheepId: any) => {
  const dbResult = await SheepApi.aggregate([
    { $match: { _id: new ObjectId(sheepId) } },
  ...
  //console.log('DB Result for sheep query', dbResult, POULTRY_SCHEMA_ID);
  return postProcess(dbResult, sheepId);
};

const postProcess = (dbResult: any, sheepId: any) => {
  try {
    const result = dbResult.map((item: any) => {
      ...
      item.productionsystemevents.map((event: any) => {
        if (
          event.category === 'Sheep Production System' &&
          event.details.sheep.some((sheep: any) => sheep.id === sheepId)
        ) {
          const modEvent = {
            _id: event._id.toString(),
            name: event.name,
    ...
    console.log('ERROR in sheepDetailsViewQuery:postProcess', e);
  }
};
